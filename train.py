...
#import tflib
#import tflib.tflibfunc
#import tflib.mnist
#import tflib.plot
#import tflib.save_images
#import tflib.ops.batchnorm
#import tflib.ops.allfunctions
...
class MnistWganInv(object):
    ...
    def generate(self, z):
        ...
        output = ops.Linear('Generator.Input', self.z_dim,
                                         self.latent_dim * 64, z)
        output = relu(output)
        output = tf.reshape(output, [-1, self.latent_dim * 4, 4, 4])  # 4 x 4

        output = ops.Deconv2D('Generator.2', self.latent_dim * 4,
                                             self.latent_dim * 2, 5, output)
        output = relu(output)  # 8 x 8
        output = output[:, :, :7, :7]  # 7 x 7

        output = ops.Deconv2D('Generator.3', self.latent_dim * 2,
                                             self.latent_dim, 5, output)
        output = relu(output)  # 14 x 14

        output = ops.Deconv2D('Generator.Output',
                                             self.latent_dim, 1, 5, output)
        output = sigmoid(output)  # 28 x 28

        if self.gen_params is None:
            self.gen_params = ops.params_with_name('Generator')
	...
	output = l_relu(output)
        ...
        if self.dis_params is None:
            self.dis_params = ops.params_with_name('Discriminator')
	...
	output = l_relu(output)  #our own relu
        #output = tf.nn.leaky_relu(output)  # 14 x 14
	...
	output = l_relu(output)
        ...
        if self.inv_params is None:
            self.inv_params = ops.params_with_name('Inverter')
...
if __name__ == '__main__':
    ...
    # dataset iterator
    train_gen, dev_gen, test_gen = ops.load(args.batch_size, args.batch_size)
    ...
    with tf.Session() as session:
        ...
        for iteration in range(args.iterations):
            ...
            ops.plot('train gen cost', gen_cost)
            ops.plot('train dis cost', dis_cost)
            ops.plot('train inv cost', inv_cost)
            ...
            if iteration % 1000 == 999:
                ...
                ops.plot('dev dis cost', np.mean(dev_dis_cost_lst))
                ops.plot('dev inv cost', np.mean(dev_inv_cost_lst))

            if iteration < 5 or iteration % 100 == 99:
                ops.flush(os.path.join(args.output_path, 'models'))
